namespace ScratchProjects.Sorting
{
    ...
    public class WeightedList<TKey> : ICollection<TKey>
    {
        ...
        public bool Remove(TKey item, bool preserveAtZero)
        {
            if (item == null)
                throw new ArgumentNullException(nameof(item), "entries must not be null");
        ...
        {
            return weights.TryGetValue(key, out int weight) ? weight : 0;
        }
