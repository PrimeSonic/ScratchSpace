namespace EasyMarkup.Core
{
    ...
    public class Deque<T> : ICollection, IReadOnlyCollection<T>
    {
        ...
        private void ArrayCopy(Array array, int targetArrayStartingIndex)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(targetArrayStartingIndex), targetArrayStartingIndex, "Starting index is less than zero or greater than the target array size");
            }
            ...
            {
                throw new ArgumentException("The number of elements in the source deque is greater than the available space " + "from starting index to the end of the destination array.");
            }
